...
class Config:
    ...
    LOGGING_BUCKET = os.environ.get(
        "LOGGING_BUCKET", "kf-s3-data-logging-bucket"
    )

    INVENTORY_DEST = os.environ.get("INVENTORY_DEST", LOGGING_BUCKET)

    DR_LOGGING_BUCKET = os.environ.get(
    ...
    if "BUCKET_SERVER_SECRET" in os.environ:
        ...
        if (
            bucket_token
            and "data" in bucket_token
            and "token" in bucket_token["data"]
        ):
